...
- Drives archive: every hard disk and SSD of the company (LION 01, SIMBA 02, TML 05…) shown as a shelf, four to a row, each card giving the name, a bar for how full it is, capacity, free space, where it is and how many projects are on it. Click a disk and its contents open underneath, full width and readable, where you add, edit and remove entries and the disk itself. Searching switches to answering the real question: one row per project found, saying which disk it is on, and a click on that disk name opens it. Visible only to the people the administrators allow (Team → permissions → Drives archive)
- Whiteboard (hidden for now, the tab is switched off in src/pages/Project.jsx with SHOW_WHITEBOARD; the page still answers at /p/<id>/whiteboard): a brainstorm board inside every project. Sticky notes you write on and drag, dashed groups to gather them, and arrows that connect two notes and follow them when you move them. Drag the background to pan, ctrl and scroll or the buttons to zoom, Fit frames everything. Colours from the toolbar apply to the selected item, or set the colour of the next note. It follows the Files & notes permission, and because everyone edits the same project document the board updates live for the whole team like the rest of the project. No drawing pen and no images on it, on purpose: see HANDOVER.
...
- Share (sidebar): send a rough cut, a prefinal, a final cut or the final files as a page with the company logo instead of a bare transfer link. Paste the SwissTransfer or WeTransfer link you already made, pick the stage and the version, add the transfer's password and expiry, a note and a date to answer by; for final files the credits come ready from the project's cast and crew and the file list from its Post tab. You get a link to send however you like, plus Copy for email. The client opens it without any account, downloads, and can Approve or Ask for changes with a note, which appears on your Share list. Share is its own permission module, so you decide per person who may send links: none hides it, view shows the sent links and the clients' answers, edit sends and deletes. Everyone starts at none until you give it. The list shows **every public link the company has out**, deliveries and call sheets together, with a filter; whether each one was opened and when; and a **Close** button that kills a link without deleting it, plus a date for it to close itself. Besides cuts you can send a **treatment** or a **lookbook** through the same page. **Status page**: one living page per project for the client, with where the job stands, what happens next and what you need from them; the link never changes, so they keep it and you update it. **Run `supabase/deliveries.sql`, `supabase/share_access.sql` and `supabase/share_track.sql` once** to enable the replies, the permission and the open counter.
- Home is three blocks now: the team across the top, then Projects and Calendar side by side at the same height. Needs attention, By status, By type and the Finance summary were removed; the numbers behind them live on the Tasks and Finance pages.
- Home can be rearranged: press **Arrange**, then drag a block where you want it or use the up and down arrows, and press Done. Reset order puts it back. The order is saved on that device only, like the theme and the text size, so your phone and your desktop can be laid out differently.
- Not available is an administrator's call: only they can mark a day off, and they mark it **for whoever is away**, picking the person in the event. Everyone still sees who is off, on Home and in the calendar. Nobody else can add, change or delete one, and the type no longer appears in the event form for anyone else.
- Home opens with **the team**: everyone left to right as large round photos (initials until they upload one), with no heading above them, just the day being shown. Anyone who marked themselves Not available in the calendar is greyed out and crossed through for the day you are looking at, and the strip follows whichever day you pick in the mini calendar below it. Click a face to open that person's profile. Everyone sees it.
- Member profiles: every teammate has a profile page with a photo, position, department, phone, a few words and a birthday. **My profile** in the sidebar is where you fill in your own; administrators can edit anyone's from Team, where each row now shows the photo and opens the profile. Chat shows the photo next to each message and the name opens the profile. Each person chooses whether the whole team or only administrators see their phone and birthday; name, photo, position and the few words are always visible. Name and email stay administrator-managed in Team. **Run `supabase/profiles.sql` once in the Supabase SQL Editor** to add the column and the `set_my_profile()` function that lets a member write their own profile without being able to touch their role or permissions.
- Settings additions: **emergency numbers** (Ambulance 166 and Fire brigade 199 by default, editable) and **standing production contacts** printed on every call sheet and on the public share link; **Greek public holidays** in both calendars, the movable ones following Orthodox Easter; an automatic **project code** like TML-2026-001 with the prefix you choose; the **language the AI writes in** (Greek or English); automatic **notices** when a task is assigned to you and when a chat message arrives, each switchable, with messages from the same person collapsing into one counted pop-up; and the **financial year start month** in Finance settings, which every year total and year tab now follows.
...
2. SQL Editor → New query → paste all of `supabase/schema.sql` → Run. Then the same with `supabase/storage.sql` (photo bucket) `supabase/library.sql` (company library), `supabase/todos.sql` (general tasks), `supabase/audio.sql` (song files), `supabase/files.sql` (project files), `supabase/finance.sql` (administrators-only finance) `supabase/chat.sql` (team chat) `supabase/notices.sql` (notices) `supabase/worklog.sql` (personal work logs), `supabase/drives.sql` (drives archive) `supabase/shares.sql` (public call sheet links), `supabase/activity.sql` (activity log), `supabase/deliveries.sql` (client replies on a delivery page) `supabase/share_access.sql` (only members with the Share permission may send delivery links) and `supabase/share_track.sql` (counts openings, closes a link by hand or on a date).
