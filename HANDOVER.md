...
Alex: "when you open a new project only the Overview tab should exist, and I switch on the ones I want." `freshProject()` in Dashboard.jsx (both New project buttons) sets `hiddenTabs = allHideable(project)`. `hiddenAfterCategory()` in src/lib/tabs.js adds the tabs a category switch brings in, so they start hidden too (form only). The chips live in `TabPicker` (exported from Dashboard.jsx), used by the project form only: a strip of them on the Overview shipped in #43 and was taken out the same day because Alex found people would tap the chips instead of the real tabs. Existing projects are untouched: still a hide-list, nothing stored means every tab.
